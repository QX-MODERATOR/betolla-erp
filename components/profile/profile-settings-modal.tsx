...
const STAFF_ACCOUNTS = [
  { username: "gm", label: "المدير العام", enLabel: "General Manager", icon: "🏛️" },
  { username: "admin", label: "مسؤول النظام (IT)", enLabel: "IT Admin", icon: "👑" },
  { username: "sales.manager", label: "مديرة المبيعات", enLabel: "Sales Mgr", icon: "💼" },
  { username: "rahma", label: "رحمة (مبيعات)", enLabel: "Rahma (Sales)", icon: "✨" },
  { username: "marketing.mgr", label: "مدير التسويق", enLabel: "Marketing Mgr", icon: "📢" },
  { username: "marketing", label: "تسويق", enLabel: "Marketing", icon: "🎯" },
  { username: "zaid", label: "زيد (المدير المالي)", enLabel: "Zaid (Finance)", icon: "🧮" },
  { username: "hr", label: "مديرة الموارد البشرية - عمليات", enLabel: "HR & Ops", icon: "👥" },
  { username: "diya", label: "ضياء (مدير سائقين التوصيل)", enLabel: "Diya (Dispatch)", icon: "🚚" },
  { username: "khalid", label: "خالد (سائق توصيل)", enLabel: "Khalid (Driver)", icon: "📦" },
  { username: "ali", label: "علي (سائق توصيل)", enLabel: "Ali (Driver)", icon: "📦" },
];
  ...
  return (
    ...
    >
      <div className="relative w-full max-w-xl bg-gradient-to-b from-[#160f02] via-[#1d1405] to-[#160f02] border border-[#554625] rounded-3xl shadow-2xl shadow-black/90 p-4 sm:p-7 text-[#f4e5d0] ring-1 ring-[#9e8959]/25 my-4 sm:my-8 max-h-[92vh] flex flex-col no-scrollbar hide-scrollbar animate-modalSlideUp">
        ...
        {/* Admin Account Switcher: Allows Admin to toggle editing staff profiles */}
        {isAdmin && (
          <div className="p-2 bg-[#120c02] rounded-2xl border border-[#3d3016] text-xs mt-3 shrink-0">
            <div className="flex items-center justify-between gap-2 mb-1.5 px-1">
              <span className="text-[#9e8959] text-[11px] font-bold flex items-center gap-1.5">
                <Crown className="w-3.5 h-3.5 text-[#9e8959]" />
                <span>{isArabic ? "إدارة حسابات فريق العمل (Admin):" : "Manage Team Accounts:"}</span>
              </span>
              <span className="text-[10px] text-[#a3998b]">
                {isArabic ? "اختر الحساب للتعديل" : "Select account"}
              </span>
            </div>
            <div className="flex items-center gap-1.5 overflow-x-auto pb-1 no-scrollbar hide-scrollbar">
              {STAFF_ACCOUNTS.map((staff) => {
                const isCurrent = profile.username === staff.username;
                return (
                  <button
                    key={staff.username}
                    type="button"
                    onClick={() => {
                      switchProfile(staff.username);
                      setFormError(null);
                      setFormSuccess(null);
                    }}
                    className={`px-2.5 sm:px-3 py-1.5 rounded-xl font-bold transition flex items-center gap-1.5 cursor-pointer text-xs shrink-0 whitespace-nowrap active:scale-95 ${
                      isCurrent
                        ? "bg-gradient-to-r from-[#9e8959] to-[#c28a40] text-[#160f02] shadow-md shadow-[#9e8959]/25"
                        : "text-[#a3998b] hover:text-[#f4e5d0] bg-[#1a1204] border border-[#3d3016] hover:border-[#554625]"
                    }`}
                  >
                    <span>{staff.icon}</span>
                    <span>{isArabic ? staff.label : staff.enLabel}</span>
                  </button>
                );
              })}
            </div>
