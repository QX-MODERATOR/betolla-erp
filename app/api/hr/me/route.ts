...
import {monthDays,type HrEmployee,type HrAttendance,type HrHoliday,type HrLeaveRequest,type HrLeaveBalance,type HrLeaveType} from '@/lib/hr';
...
export async function GET(req:Request) {
  try{const user=await businessUser(req,'/api/hr/me');
    ...
    const [today,settings,attendance,holidays,types,requests,teamRequests]=await Promise.all([
