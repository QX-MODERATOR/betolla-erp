import {businessUser,businessRpc,businessFailure,requestKey,readBody,preparePayment,preparePaymentReversal} from '@/lib/business-server';
...
export async function POST(req:Request) {
  try{const user=await businessUser(req,'/api/finance'),key=requestKey(req),body=await readBody(req);
