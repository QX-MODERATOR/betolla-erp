...
// here — the real caller is n8n (server-to-server), not a browser — so this
// header is the actual gate against unauthenticated order injection.
...
serve(async (req: Request) => {
  ...
  try {
    ...
    const rawText = String(body.rawText || body.text || body.message || body.body || "").slice(0, 4000);
