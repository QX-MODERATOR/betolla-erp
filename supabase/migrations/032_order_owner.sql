-- Same signature, replaces the migration 010 version. Three changes only:
--   * p_data.owner_account_id (set by the app) makes the order belong to the sales rep it is for,
--     when a manager enters it on the rep's behalf: the rep then sees it in her order list and can
--     move its status (business_status scopes reps by owner). Without it the creator owns it, as before.
--   * p_data.reuse_phone: without an explicit customer_id, an order for a phone number that belongs to
--     exactly one existing customer (within p_data.scope_rep for a sales rep) reuses that customer
--     instead of creating a duplicate. Decided here, after the idempotency check, so a retried
--     request sends the same payload and replays.
--   * a missing order/due date defaults to today's Amman date instead of the database's UTC date.
-- Requires migration 027 (business_amman_today).
BEGIN;
